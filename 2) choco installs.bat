:: Development tools
cinst -y nodejs.install
cinst -y ruby
::cinst -y fiddler
::cinst -y westwindwebsurge
::cinst -y compass
::cinst -y scriptcs
::cinst -y putty
::cinst -y rdcman
::cinst -y yeoman
:: cinst -y curl

:: SMTP server mocks
::cinst -y smtp4dev

:: Source control
::cinst -y git.install
::cinst -y githubforwindows
::cinst -y sourcetree

:: IDEs / text editors
::cinst -y vscode
::cinst -y notepadplusplus.install
::cinst -y brackets
::cinst -y sublimetext3
::cinst -y webstorm

:: IIS
::cinst -y urlrewrite

:: Windows Installers
::cinst -y webpi

:: .NET development tools
::cinst -y linqpad
::cinst -y nuget.commandline
::cinst -y nugetpackageexplorer
::cinst -y nugetpackagemanager
::cinst -y typescript.vs
::cinst -y resharper

:: .NET reflection tools
::cinst -y dotpeek
::cinst -y ilspy

:: Merge tools
::cinst -y beyondcompare
::cinst -y winmerge
::cinst -y p4merge

:: Databases
::cinst -y mongodb
::cinst -y sqlserver2014express

:: Browsers
::cinst -y google-chrome-x64
::cinst -y firefox
::cinst -y opera
::cinst -y googlechrome.canary
::cinst -y maxthon

:: Media players and editors
::cinst -y spotify
::cinst -y handbrake.install
::cinst -y vlc
::cinst -y audacity
::cinst -y itunes
::cinst -y kodi

:: Gaming
::cinst -y steam
::cinst -y unity
::cinst -y unitywebplayer

:: Cloud storage
::cinst -y dropbox
::cinst -y googledrive
::cinst -y onedrive

:: Note taking
::cinst -y evernote

:: Instant messaging / chat clients
::cinst -y skype
::cinst -y hipchat

:: Downloaders
::cinst -y sabnzbd

:: Security softare
::cinst -y avastfreeantivirus
::cinst -y avastbrowsercleanup

:: Misc utils
::cinst -y imgburn
::cinst -y keepass.install
::cinst -y keepass-keepasshttp
::cinst -y 7zip.install
::cinst -y winrar
::cinst -y clover
::cinst -y f.lux
::cinst -y linkshellextension
::cinst -y windowslivewriter
::cinst -y speedfan
::cinst -y sysinternals
::cinst -y ccleaner
::cinst -y foxitreader
::cinst -y paint.net
::cinst -y sublimetext3
::cinst -y teamviewer

@echo off
echo .
echo .
echo .
echo Complete. Check output to see if all 'cinst' statements succeeded.
pause
